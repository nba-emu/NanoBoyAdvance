...
void PPU::InitBackground() {
  ...
  // // test
  // for(int x = 0; x < 240; x++) {
  //   output[frame][mmio.vcount * 240 + x] = 0;
  // }
}
...
void PPU::DrawBackground() {
  ...
  for(int i = 0; i < cycles; i++) {
    ...
    if(mode <= 1) {
      ...
      // @todo: do not render if BG is BG2 or BG3 and mode is #1
      ...
      // @todo: research how the map-fetch is triggered in hardware.
      if(((draw_x + bghofs_mod_8) & 7) == 0) {
        ...
        // @todo: using draw_x breaks when using sub-tile scrolling
        const uint grid_x = bghofs_div_8 + (draw_x >> 3) - 1U;
    ...
    int m0id = cycle & 3;
    if(bg.text[m0id].piso.remaining > 0) {
      bg.text[m0id].piso.data >>= 4;
      bg.text[m0id].piso.remaining--;
    }
